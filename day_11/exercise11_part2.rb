...
# counter of steps
first_step = 0
# according to task content: make a loop until whole board will be synchonized = all zeros
loop do
  # count steps
  first_step += 1 
...
# so on each row (map! to change immediately elements on grid)
  ...
  # due to resseting flashes we cant use flashes.all?(0)
  # but,lucky us - there are always (input/example) 100 fields = 10x10
  # so on columns and rows - we need count 100
  break if flashes.count == 100
end
# number of steps
p first_step
# => 334
